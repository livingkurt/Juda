...
export const parseDroppableId = droppableId => {
  ...
  // Backlog priority section - format: "backlog-priority|{priority}"
  if (droppableId.startsWith("backlog-priority|")) {
    const priority = droppableId.split("|")[1];
    return { type: "backlog-priority", priority: priority === "none" ? null : priority };
  }
  ...
  // Kanban column - format: "kanban-column|{status}"
  if (droppableId.startsWith("kanban-column|")) {
    const status = droppableId.split("|")[1];
    return { type: "kanban-column", status };
  }
...
export const createDroppableId = {
  backlog: () => "backlog",
  backlogPriority: priority => `backlog-priority|${priority || "none"}`,
  ...
  kanbanColumn: status => `kanban-column|${status}`,
};
...
export const createDraggableId = {
  ...
  kanban: (taskId, status) => `task-${taskId}-kanban-${status}`,
};
...
export const extractTaskId = draggableId => {
  ...
  // Handle kanban format: "task-{taskId}-kanban-{status}"
  if (withoutPrefix.includes("-kanban-")) {
    const match = withoutPrefix.match(/^(.+)-kanban-/);
