...
import { broadcastChange, ENTITY_TYPES, OPERATIONS } from "./sse/broadcast.js";
...
export function getClientIdFromRequest(request) {
  return request.headers.get("X-Client-ID") || null;
}

/**
 * Create broadcast helpers for an entity type
 * @param {string} entityType - The entity type (from ENTITY_TYPES)
 * @returns {object} - Object with broadcast helper methods
 */
export function withBroadcast(entityType) {
  return {
    onCreate: (userId, data, clientId = null) => broadcastChange(userId, entityType, OPERATIONS.CREATE, data, clientId),

    onUpdate: (userId, data, clientId = null) => broadcastChange(userId, entityType, OPERATIONS.UPDATE, data, clientId),

    onDelete: (userId, id, clientId = null) => broadcastChange(userId, entityType, OPERATIONS.DELETE, { id }, clientId),

    onBatchCreate: (userId, items, clientId = null) =>
      broadcastChange(userId, entityType, OPERATIONS.BATCH_CREATE, { items }, clientId),

    onBatchDelete: (userId, ids, clientId = null) =>
      broadcastChange(userId, entityType, OPERATIONS.BATCH_DELETE, { ids }, clientId),

    onBatchUpdate: (userId, items, clientId = null) =>
      broadcastChange(userId, entityType, OPERATIONS.BATCH_UPDATE, { items }, clientId),

    onReorder: (userId, items, clientId = null) =>
      broadcastChange(userId, entityType, OPERATIONS.REORDER, { items }, clientId),
  };
}

// Re-export for convenience
export { ENTITY_TYPES, OPERATIONS };
