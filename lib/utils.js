 ...
 * - Task has a time
 * - Task is not completed on that date
 ...
 * @param {boolean} isCompleted - Whether the task is completed on that date
 */
export const isOverdue = (task, date = null, isCompleted = false) => {
  // Tasks without a time are never overdue - they can always be done
  if (!task.time) return false;

  // If task is completed on this date, it's not overdue
