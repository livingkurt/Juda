...
export const shouldShowOnDate = (task, date) => {
  ...
  if (!task.recurrence) {
    // Tasks without recurrence don't show in calendar/today views
    return false;
  }
  ...
  if (type === "none") {
    // One-time task: show on the specific date (with or without time)
    ...
    // Show only on the exact date (time is optional)
