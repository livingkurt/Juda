...
export const shouldShowOnDate = (task, date, getOutcomeOnDate = null, getLatestOutcomeOnOrBeforeDate = null) => {
  ...
  // Check if this task has an unresolved rollover
  if (getLatestOutcomeOnOrBeforeDate) {
    const latest = getLatestOutcomeOnOrBeforeDate(task.id, checkDate);
    if (latest?.outcome === "rolled_over") {
      const latestDateStr = formatLocalDate(latest.date);
      if (latestDateStr <= checkDateStr) {
        return true;
      }
    }
  } else if (getOutcomeOnDate) {
    // Walk backwards from current date to find the most recent outcome
    let checkDay = new Date(checkDate);
    // Look back up to 365 days for a rollover (prevent infinite loop)
    for (let i = 0; i < 365; i++) {
      checkDay.setDate(checkDay.getDate() - 1);
      const outcome = getOutcomeOnDate(task.id, checkDay);

      if (outcome === "rolled_over") {
        // Found a rollover - show the task on current date
        return true;
      } else if (outcome === "completed" || outcome === "not_completed") {
        // Found a completion - stop looking, task was resolved
        break;
      }
      // If outcome is null, keep looking backwards
    }
  }
  ...
  if (exceptions && Array.isArray(exceptions) && exceptions.length > 0) {
    if (exceptions.includes(checkDateStr)) {
      return false; // This date is excepted from the recurring pattern
    }
  }
  ...
  if (type === "none") {
    ...
    return checkDateStr >= taskDateStr;
  }
  ...
  switch (type) {
    ...
    case "yearly": {
      ...
      if (checkDate.getMonth() + 1 !== targetMonth) return false;
