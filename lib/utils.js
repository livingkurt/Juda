...
/**
 * Performance optimization: Create a cache for shouldShowOnDate results
 * This prevents recalculating recurrence patterns for the same task+date combinations
 */
const shouldShowOnDateCache = new Map();
const CACHE_SIZE_LIMIT = 10000; // Limit cache size to prevent memory issues

const createCacheKey = (task, date) => {
  const dateKey = date instanceof Date ? date.toDateString() : new Date(date).toDateString();
  const recurrenceKey = task.recurrence
    ? `${task.recurrence.type}-${task.recurrence.startDate || ""}-${task.recurrence.endDate || ""}-${JSON.stringify(task.recurrence.days || [])}-${JSON.stringify(task.recurrence.dayOfMonth || [])}-${task.recurrence.interval || ""}-${JSON.stringify(task.recurrence.exceptions || [])}-${JSON.stringify(task.recurrence.additionalDates || [])}`
    : "no-recurrence";
  return `${task.id}-${dateKey}-${recurrenceKey}`;
};

/**
 * Check if a task should be shown on a specific date based on its recurrence pattern
 * Performance optimized with caching to avoid repeated calculations
 * @param {Object} task - The task object
 * @param {Date|string} date - The date to check
 * @param {boolean} useCache - Whether to use cache (default: true)
 * @returns {boolean} Whether the task should be shown on this date
 */
export const shouldShowOnDate = (task, date, useCache = true) => {
  ...
  let cacheKey = null;
  if (useCache) {
    cacheKey = createCacheKey(task, date);
    if (shouldShowOnDateCache.has(cacheKey)) {
      return shouldShowOnDateCache.get(cacheKey);
    }
  }

  const { type, days, dayOfMonth, interval, startDate, endDate, additionalDates, exceptions } = task.recurrence;
  ...
  // Check if this date is in exceptions list (for "this occurrence only" edits)
  if (exceptions && Array.isArray(exceptions) && exceptions.length > 0) {
    if (exceptions.includes(checkDateStr)) {
      const result = false; // This date is excepted from the recurring pattern
      if (useCache && cacheKey) {
        shouldShowOnDateCache.set(cacheKey, result);
      }
      return result;
    }
  }
  ...
  if (type === "none") {
    ...
    // Show task on its scheduled date and all dates after (for overdue behavior)
    // This allows tasks to "roll over" to the next day if not completed
    const result = checkDateStr >= taskDateStr;
    if (useCache && cacheKey) {
      shouldShowOnDateCache.set(cacheKey, result);
    }
    return result;
  ...
  switch (type) {
    case "daily":
      ...
      break;
    case "weekly":
      result = days?.includes(dayOfWeek) || false;
      break;
    case "monthly": {
      ...
      if (task.recurrence.weekPattern) {
        ...
        const dateOrdinal = getOrdinalWeekOfMonth(checkDate);
      ...
      if (task.recurrence.interval && startDate) {
        ...
        const monthsDiff = getMonthsDifference(start, checkDate);
    ...
    case "yearly": {
      ...
      if (checkDate.getMonth() + 1 !== targetMonth) {
        result = false;
        break;
      }
      ...
      if (task.recurrence.weekPattern) {
        ...
        if (checkDate.getDate() !== task.recurrence.dayOfMonth) return false;
      }
      ...
      if (task.recurrence.interval && startDate) {
        const startParts = startDate.split("T")[0].split("-").map(Number);
        const start = new Date(startParts[0], startParts[1] - 1, startParts[2]);
        const yearsDiff = getYearsDifference(start, checkDate);
    ...
    case "interval": {
      ...
      result = diffDays >= 0 && diffDays % interval === 0;
      break;
    }
    default:
      result = true;
  ...
  if (useCache && cacheKey) {
    ...
    if (shouldShowOnDateCache.size >= CACHE_SIZE_LIMIT) {
      const firstKey = shouldShowOnDateCache.keys().next().value;
      shouldShowOnDateCache.delete(firstKey);
    }
    shouldShowOnDateCache.set(cacheKey, result);
  }
...
/**
 * Clear the shouldShowOnDate cache
 * Useful when tasks are updated and cache might be stale
 */
export const clearShouldShowOnDateCache = () => {
  shouldShowOnDateCache.clear();
};
 ...
 */
/**
 * Get the display color for a task (from first tag)
 * Maps stored color to current theme palette
 * @param {Object} task - Task object with tags array
 * @param {Object} theme - Current theme object (optional, for theme mapping)
 * @param {string} colorMode - Current color mode ('light' or 'dark')
 * @returns {string|null} Hex color or null
 */
export function getTaskDisplayColor(task, theme = null, colorMode = "dark") {
  if (task?.tags && task.tags.length > 0) {
    ...
    if (theme && theme.colors && theme.colors[colorMode]) {
      const { mapColorToTheme } = require("./themes.js");
