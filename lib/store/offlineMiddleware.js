...
import { putInStore, deleteFromStore, getByIndex, STORES } from "@/lib/db/indexedDB";
...
const ENDPOINT_TO_ENTITY = {
  ...
  batchCreateCompletions: ENTITY_TYPES.COMPLETION,
  deleteCompletion: ENTITY_TYPES.COMPLETION,
  batchDeleteCompletions: ENTITY_TYPES.COMPLETION,
...
const ENDPOINT_TO_STORE = {
  ...
  batchCreateCompletions: STORES.COMPLETIONS,
  deleteCompletion: STORES.COMPLETIONS,
  batchDeleteCompletions: STORES.COMPLETIONS,
...
// Normalize completion date to UTC midnight ISO string
function normalizeCompletionDate(date) {
  if (!date) {
    const now = new Date();
...
async function deleteCompletionByTaskDate(taskId, date) {
  if (!taskId || !date) return;

  const normalizedDate = normalizeCompletionDate(date);
  const matches = await getByIndex(STORES.COMPLETIONS, "taskId_date", [taskId, normalizedDate]);
  if (!matches || matches.length === 0) return;

  await Promise.all(matches.map(match => deleteFromStore(STORES.COMPLETIONS, match.id)));
}

// Map stores to their required key paths
const STORE_KEY_PATHS = {
  [STORES.TASKS]: "id",
  [STORES.SECTIONS]: "id",
  [STORES.TAGS]: "id",
  [STORES.COMPLETIONS]: "id",
  [STORES.FOLDERS]: "id",
  [STORES.SMART_FOLDERS]: "id",
  [STORES.PREFERENCES]: "userId",
  [STORES.WORKOUT_PROGRAMS]: "id",
};
...
async function updateOfflineCache(endpointName, args, response) {
  ...
  try {
    if (operation === SYNC_OPERATIONS.DELETE) {
      if (store === STORES.COMPLETIONS) {
        if (Array.isArray(args)) {
          await Promise.all(args.map(item => deleteCompletionByTaskDate(item?.taskId, item?.date)));
        } else {
          await deleteCompletionByTaskDate(args?.taskId, args?.date);
        }
        return;
      }

      const key = args?.id || args;
      if (key) {
        await deleteFromStore(store, key);
      }
    } else if (response) {
      // Validate that response has the required key path
      const keyPath = STORE_KEY_PATHS[store];
      if (!keyPath) {
        console.warn(`No key path defined for store: ${store}`);
        return;
      }

      // Check if response is an array (batch operations)
      if (Array.isArray(response)) {
        // Filter out items without required key path
        const validItems = response.filter(item => item && item[keyPath] != null);
        if (validItems.length > 0) {
          const { bulkPutInStore } = await import("@/lib/db/indexedDB");
          await bulkPutInStore(store, validItems);
        }
      } else if (Array.isArray(response?.completions)) {
        const validItems = response.completions.filter(item => item && item[keyPath] != null);
        if (validItems.length > 0) {
          const { bulkPutInStore } = await import("@/lib/db/indexedDB");
          await bulkPutInStore(store, validItems);
        }
      } else if (response[keyPath] != null) {
        // Single item with required key path
        await putInStore(store, response);
      } else {
        console.warn(`Response missing required key path '${keyPath}' for store '${store}':`, response);
      }
    }
  } catch (error) {
