...
export const tasksApi = baseApi.injectEndpoints({
  endpoints: builder => ({
    ...
    getNonRecurringTasks: builder.query({
      query: ({ page = 1, limit = 25, search, tagIds, priorities, completionTypes, statuses } = {}) => {
        const params = new URLSearchParams();
        params.append("page", String(page));
        params.append("limit", String(limit));
        if (search) params.append("search", search);
        if (tagIds?.length) params.append("tagIds", tagIds.join(","));
        if (priorities?.length) params.append("priorities", priorities.join(","));
        if (completionTypes?.length) params.append("completionTypes", completionTypes.join(","));
        if (statuses?.length) params.append("statuses", statuses.join(","));
        return `/tasks/non-recurring?${params.toString()}`;
      },
      providesTags: result =>
        result?.tasks
          ? [...result.tasks.map(t => ({ type: "Task", id: t.id })), { type: "Task", id: "NON_RECURRING" }]
...
export const {
  ...
  useGetNonRecurringTasksQuery, // Non-recurring tasks (HistoryTab Non-Recurring view)
