...
export const tasksApi = baseApi.injectEndpoints({
  endpoints: builder => ({
    ...
    // GET list templates and active list instances
    getListTasks: builder.query({
      ...
      providesTags: result => {
        ...
        return [
          ...templates.map(t => ({ type: "Task", id: t.id })),
          ...instances.map(t => ({ type: "Task", id: t.id })),
          { type: "Task", id: "LISTS" },
        ];
      },
    }),

    // Create a list instance from a template
    useListTemplate: builder.mutation({
      query: payload => ({
        url: "/tasks/lists/use",
        method: "POST",
    ...
    // Canonical list item library
    getListItems: builder.query({
      query: search => (search ? `/list-items?search=${encodeURIComponent(search)}` : "/list-items"),
      providesTags: result =>
        result
          ? [...result.map(item => ({ type: "Task", id: `LIST_ITEM_${item.id}` })), { type: "Task", id: "LIST_ITEMS" }]
          : [{ type: "Task", id: "LIST_ITEMS" }],
    }),
    ...
    createListItem: builder.mutation({
      ...
      invalidatesTags: [{ type: "Task", id: "LIST_ITEMS" }],
    }),
    ...
    updateTask: builder.mutation({
      ...
      invalidatesTags: (result, error, { id }) => [
        ...
        { type: "Task", id: "LISTS" }, // Invalidate lists tab
      ],
    }),
    ...
    deleteTask: builder.mutation({
      ...
      invalidatesTags: [
        ...
        { type: "Task", id: "LISTS" }, // Invalidate lists tab when deleting
      ],
    }),
...
export const {
  ...
  useGetListTasksQuery, // List templates + instances
  useUseListTemplateMutation, // Clone list template into active instance
  useGetListItemsQuery, // Canonical list item library
  useCreateListItemMutation, // Add canonical list item
