...
export const completionsApi = baseApi.injectEndpoints({
  ...
  endpoints: builder => ({
    ...
    createCompletion: builder.mutation({
      query: ({ taskId, date, outcome = "completed", note, selectedOptions, time, startedAt, completedAt }) => ({
        url: "/completions",
        method: "POST",
        body: JSON.stringify({
          taskId,
      ...
      // OPTIMISTIC UPDATE - useDeferredValue handles the deferred rendering
      onQueryStarted(
        { taskId, date, outcome = "completed", note, selectedOptions, time, startedAt, completedAt },
        ...
        for (const { endpointName, originalArgs } of cachedQueries) {
          if (endpointName === "getCompletions") {
            ...
            patches.push(patch);
          } else if (endpointName === "getCompletionsByDateRange") {
            const patch = dispatch(
              completionsApi.util.updateQueryData("getCompletionsByDateRange", originalArgs, draft => {
                if (!Array.isArray(draft)) return;
                const idx = draft.findIndex(
                  c => c.taskId === taskId && new Date(c.date).toISOString() === normalizedDate
                );
                if (idx !== -1) draft.splice(idx, 1);
                draft.push(newCompletion);
              })
            );
            patches.push(patch);
          }
