...
const upsertTaskInTree = (tasks, data) => {
    ...
    for (const task of tasks) {
      if (task.id === data.parentId) {
        ...
        if (idx === -1) {
          ...
          subtasks.splice(idx, 1, mergeTask(subtasks[idx]));
        }
  ...
  if (index === -1) {
    ...
    tasks.splice(index, 1, mergeTask(tasks[index]));
  }
  return true;
};
...
const taskExistsInTree = (tasks, id) => {
  if (!Array.isArray(tasks) || !id) return false;
  for (const task of tasks) {
    if (task.id === id) return true;
    if (task.subtasks?.length && taskExistsInTree(task.subtasks, id)) {
      return true;
    }
  }
  return false;
...
const tasksContainAnyId = (tasks, ids) => {
  if (!Array.isArray(ids) || ids.length === 0) return false;
  return ids.some(id => taskExistsInTree(tasks, id));
};
...
const shouldIncludeInEndpoint = (endpointName, args, task) => {
  switch (endpointName) {
    ...
    case "getListTasks":
      return task.completionType === "list" && !task.parentId;
...
const getTaskPayload = data => {
  if (!data) return { task: null, previousTask: null };
  if (data.task) {
    return { task: data.task, previousTask: data.previousTask ?? null };
  }
  return { task: data, previousTask: data.previousTask ?? null };
};

const isSseDebugEnabled = () => {
  if (typeof window === "undefined") return false;
  return window.localStorage?.getItem("sseDebug") === "true";
};

const logTaskCacheDecision = info => {
  if (!isSseDebugEnabled()) return;
  console.warn("[SSE] Task cache decision:", info);
};
...
const entityHandlers = {
  ...
  ListItem: {
    api: listApi,
    queryName: "getListItems",
    handlers: {
      create: (draft, data) => {
        if (!draft.find(i => i.id === data.id)) draft.push(data);
      },
      update: (draft, data, serverTimestamp) => {
        if (!shouldApplyUpdate("ListItem", data.id, serverTimestamp)) return;
        const idx = draft.findIndex(i => i.id === data.id);
        if (idx !== -1) draft[idx] = { ...draft[idx], ...data };
      },
      delete: (draft, data) => {
        const idx = draft.findIndex(i => i.id === data.id);
        if (idx !== -1) draft.splice(idx, 1);
        lastUpdateTimestamps.delete(`ListItem:${data.id}`);
      },
    },
  },
  ListTemplate: {
    api: listApi,
    queryName: "getListTemplates",
    handlers: {
      create: (draft, data) => {
        if (!draft.find(t => t.id === data.id)) draft.push(data);
      },
      update: (draft, data, serverTimestamp) => {
        if (!shouldApplyUpdate("ListTemplate", data.id, serverTimestamp)) return;
      ...
      delete: (draft, data) => {
        const idx = draft.findIndex(t => t.id === data.id);
        if (idx !== -1) draft.splice(idx, 1);
        lastUpdateTimestamps.delete(`ListTemplate:${data.id}`);
      },
      reorder: (draft, data) => {
        const orderMap = new Map(data.items.map(item => [item.id, item.order]));
        for (let i = 0; i < draft.length; i++) {
  ...
  ListInstance: {
    api: listApi,
    queryName: "getListInstances",
    handlers: {
      create: (draft, data) => {
        if (!draft.find(i => i.id === data.id)) draft.unshift(data);
      },
      update: (draft, data, serverTimestamp) => {
        if (!shouldApplyUpdate("ListInstance", data.id, serverTimestamp)) return;
        const idx = draft.findIndex(i => i.id === data.id);
        if (idx !== -1) draft[idx] = { ...draft[idx], ...data };
      },
      delete: (draft, data) => {
        const idx = draft.findIndex(i => i.id === data.id);
        if (idx !== -1) draft.splice(idx, 1);
        lastUpdateTimestamps.delete(`ListInstance:${data.id}`);
      },
    },
  },
  Preferences: {
...
export function initializeSSESync(store) {
  ...
  const unsubscribe = sseManager.subscribe(message => {
    ...
    if (message.type === "offlineSync") {
      ...
      store.dispatch(listApi.util.invalidateTags(["ListItem", "ListTemplate", "ListInstance"]));
      return;
    }
    ...
    if (entityType === "Task") {
      ...
      const { task, previousTask } = getTaskPayload(data);
      const taskId = task?.id ?? data?.id ?? null;

      taskQueries.forEach(query => {
        const queryData = query?.data;
        const existsInQuery = taskId ? taskExistsInTree(queryData, taskId) : false;
        const shouldIncludeNow =
          task && !["reorder", "batchDelete", "batchCreate", "batchUpdate"].includes(operation)
            ? shouldIncludeInEndpoint(query.endpointName, query.originalArgs, task)
            : false;
        const shouldIncludePrev =
          previousTask && !["reorder", "batchDelete", "batchCreate", "batchUpdate"].includes(operation)
            ? shouldIncludeInEndpoint(query.endpointName, query.originalArgs, previousTask)
            : false;
        const reorderIds = operation === "reorder" ? data?.items?.map(item => item.id) : [];
        const batchIds = operation === "batchDelete" ? data?.ids || [] : [];
        const batchItems = operation === "batchCreate" || operation === "batchUpdate" ? data?.items || [] : [];
        const batchHasMatches =
          (operation === "batchCreate" || operation === "batchUpdate") &&
          (batchItems.some(item => shouldIncludeInEndpoint(query.endpointName, query.originalArgs, item)) ||
            tasksContainAnyId(
              queryData,
        ...
        const shouldTouch =
          operation === "delete"
            ? existsInQuery || shouldIncludePrev
            : operation === "reorder"
              ? tasksContainAnyId(queryData, reorderIds)
              : operation === "batchDelete"
                ? tasksContainAnyId(queryData, batchIds)
                : operation === "batchCreate" || operation === "batchUpdate"
                  ? batchHasMatches
                  : shouldIncludeNow || shouldIncludePrev || existsInQuery;
        ...
        store.dispatch(
          handler.api.util.updateQueryData(query.endpointName, query.originalArgs, draft => {
            ...
            if (operation === "batchDelete") {
              (data.ids || []).forEach(id => {
                removeTaskFromTree(draft, id);
                lastUpdateTimestamps.delete(`Task:${id}`);
              });
              return;
            }

            if (operation === "batchCreate" || operation === "batchUpdate") {
              (data.items || []).forEach(item => {
                const includeNow = shouldIncludeInEndpoint(query.endpointName, query.originalArgs, item);
                const exists = taskExistsInTree(draft, item.id);
                if (!includeNow) {
                  if (exists) removeTaskFromTree(draft, item.id);
                  return;
                }
                applyTaskOperation(draft, operation === "batchUpdate" ? "update" : "create", item, serverTimestamp);
              });
              return;
            }
            ...
            if (operation === "delete") {
              if (taskId) {
                removeTaskFromTree(draft, taskId);
                lastUpdateTimestamps.delete(`Task:${taskId}`);
              }
              return;
            }

            if (!shouldIncludeNow) {
              const existsInDraft = taskId ? taskExistsInTree(draft, taskId) : false;
              if (existsInDraft) {
                removeTaskFromTree(draft, taskId);
              }
              return;
            ...
            applyTaskOperation(draft, operation, task, serverTimestamp);
          })
        );
      });
