...
const initialState = {
  ...
  // View state - Tabs
  mainTabIndex: 0, // 0 = Tasks, 1 = Goals, 2 = Journal, 3 = Notes, 4 = Progress, 5 = Workout, 6 = Sleep, 7 = List, 8 = Kanban, 9 = History
  ...
  journalScrollToTaskId: null, // Task ID to scroll to and expand
  journalScrollToGoalId: null, // Goal ID to scroll to within the task

  // View state - Progress
  progressView: "daily", // "daily" | "weekly" | "monthly" | "yearly"
  progressSelectedDate: getInitialDate(),

  // View state - History
  historyMode: "recurring", // "recurring" | "non-recurring"
  historyRange: "month",
  historyPage: 0,
  historySearchTerm: "",
  historyView: "table", // "table" | "graph"
  historyGraphSort: "time", // "time" | "name" | "most-consistent" | "least-consistent"
  historyGraphOrientation: null, // null means auto (mobile=horizontal, desktop=vertical)

  // View state - History (Non-Recurring)
  nonRecurringPage: 1,
  nonRecurringSearchTerm: "",
  nonRecurringSelectedTagIds: [],
  nonRecurringSelectedPriorities: [],
  nonRecurringSelectedCompletionTypes: [],
  nonRecurringSelectedStatuses: [],
  ...
  // View state - List
  selectedListTemplateId: null,
  listViewMode: "templates", // "templates" | "instances"

  // View state - Sleep
  sleepView: "day", // "day" | "week" | "month" | "year"
  sleepSelectedDate: getInitialDate(),
  ...
  backlogSortByTag: false,
  calendarSearchTerm: "",
  calendarSelectedTagIds: [],
  notesSelectedTagIds: [],
  goalsSearchTerm: "",
  goalsSelectedTagIds: [],
  goalsSelectedYear: new Date().getFullYear(),
  goalsSelectedMonth: new Date().getMonth() + 1, // 1-12
  goalsSelectedWeekStart: getInitialWeekStartDate(),
  goalsViewType: "monthly", // "weekly" | "monthly" | "yearly"
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    openTaskDialog: (state, action) => {
      ...
      if (action.payload) {
        const { completionType, goalYear, sectionId, time, date } = action.payload;
        state.defaultCompletionType = completionType || null;
        state.defaultGoalYear = goalYear || null;
        state.defaultSectionId = sectionId || null;
        state.defaultTime = time || null;
        state.defaultDate = date || null;
      }
    },
    closeTaskDialog: state => {
      ...
      state.defaultCompletionType = null;
      state.defaultGoalYear = null;
    },
    ...
    // Alias for setSelectedDate — kept for backward compatibility
    ...
    setJournalScrollToTaskId: (state, action) => {
      state.journalScrollToTaskId = action.payload;
    },
    setJournalScrollToGoalId: (state, action) => {
      state.journalScrollToGoalId = action.payload;
    },
    clearJournalScrollTarget: state => {
      state.journalScrollToTaskId = null;
      state.journalScrollToGoalId = null;
    },

    // Progress view
    setProgressView: (state, action) => {
      state.progressView = action.payload;
    },
    setProgressSelectedDate: (state, action) => {
      const date = action.payload;
      state.progressSelectedDate = date instanceof Date ? date.toISOString() : date;
    },
    ...
    setHistoryGraphSort: (state, action) => {
      state.historyGraphSort = action.payload;
    },
    setHistoryGraphOrientation: (state, action) => {
      state.historyGraphOrientation = action.payload;
    },
    ...
    setBacklogSortByPriority: (state, action) => {
      state.backlogSortByPriority = action.payload;
    },
    toggleBacklogSortByTag: state => {
      state.backlogSortByTag = !state.backlogSortByTag;
    },
    setBacklogSortByTag: (state, action) => {
      state.backlogSortByTag = action.payload;
    },
    ...
    setNotesSelectedTagIds: (state, action) => {
      state.notesSelectedTagIds = action.payload;
    },
    addNotesSelectedTag: (state, action) => {
      if (!state.notesSelectedTagIds.includes(action.payload)) {
        state.notesSelectedTagIds.push(action.payload);
      }
    },
    removeNotesSelectedTag: (state, action) => {
      state.notesSelectedTagIds = state.notesSelectedTagIds.filter(id => id !== action.payload);
    },
    setGoalsSearchTerm: (state, action) => {
      state.goalsSearchTerm = action.payload;
    },
    setGoalsSelectedTagIds: (state, action) => {
      state.goalsSelectedTagIds = action.payload;
    },
    setGoalsSelectedYear: (state, action) => {
      state.goalsSelectedYear = action.payload;
    },
    setGoalsSelectedMonth: (state, action) => {
      state.goalsSelectedMonth = action.payload;
    },
    setGoalsSelectedWeekStart: (state, action) => {
      const date = action.payload;
      state.goalsSelectedWeekStart = date instanceof Date ? date.toISOString() : date;
    },
    setGoalsViewType: (state, action) => {
      state.goalsViewType = action.payload;
    },
    addGoalsSelectedTag: (state, action) => {
      if (!state.goalsSelectedTagIds.includes(action.payload)) {
        state.goalsSelectedTagIds.push(action.payload);
      }
    },
    removeGoalsSelectedTag: (state, action) => {
      state.goalsSelectedTagIds = state.goalsSelectedTagIds.filter(id => id !== action.payload);
    },
    ...
    // History mode (recurring vs non-recurring)
    setHistoryMode: (state, action) => {
      state.historyMode = action.payload;
    },

    // Non-recurring tasks view
    setNonRecurringPage: (state, action) => {
      state.nonRecurringPage = action.payload;
    },
    setNonRecurringSearchTerm: (state, action) => {
      state.nonRecurringSearchTerm = action.payload;
      state.nonRecurringPage = 1;
    },
    setNonRecurringSelectedTagIds: (state, action) => {
      state.nonRecurringSelectedTagIds = action.payload;
      state.nonRecurringPage = 1;
    },
    setNonRecurringSelectedPriorities: (state, action) => {
      state.nonRecurringSelectedPriorities = action.payload;
      state.nonRecurringPage = 1;
    },
    setNonRecurringSelectedCompletionTypes: (state, action) => {
      state.nonRecurringSelectedCompletionTypes = action.payload;
      state.nonRecurringPage = 1;
    },
    setNonRecurringSelectedStatuses: (state, action) => {
      state.nonRecurringSelectedStatuses = action.payload;
      state.nonRecurringPage = 1;
    },

    // Sleep view
    setSleepView: (state, action) => {
      state.sleepView = action.payload;
    },
    setSleepSelectedDate: (state, action) => {
      const date = action.payload;
      state.sleepSelectedDate = date instanceof Date ? date.toISOString() : date;
    },

    // List
    setSelectedListTemplateId: (state, action) => {
      state.selectedListTemplateId = action.payload;
    },
...
export const {
  ...
  setNotesSelectedTagIds,
  addNotesSelectedTag,
  removeNotesSelectedTag,
  setGoalsSearchTerm,
  setGoalsSelectedTagIds,
