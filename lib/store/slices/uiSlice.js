...
const initialState = {
  ...
  defaultTaskKind: null,

  // Selection state
  selectedTaskIds: [],

  // View state - Tabs
  mainTabIndex: 0, // 0 = Tasks, 1 = Goals, 2 = Journal, 3 = Notes, 4 = Progress, 5 = Workout, 6 = Sleep, 7 = Kanban, 8 = History, 9 = Lists
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    openTaskDialog: (state, action) => {
      ...
      if (action.payload) {
        const { completionType, goalYear, sectionId, time, date, taskKind } = action.payload;
        state.defaultCompletionType = completionType || null;
        state.defaultGoalYear = goalYear || null;
        state.defaultTaskKind = taskKind || null;
