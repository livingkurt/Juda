...
export const tasks = pgTable(
  "Task",
  {
    ...
    taskKind: text("taskKind").notNull().default("default"), // 'default' | 'list_template' | 'list_instance'
    listTemplateId: text("listTemplateId").references(() => tasks.id, { onDelete: "set null" }),
  ...
  table => [
    ...
    index("Task_sourceTaskId_idx").on(table.sourceTaskId),
    index("Task_taskKind_idx").on(table.taskKind),
    index("Task_listTemplateId_idx").on(table.listTemplateId),
  ]
);
...
// ListItem table - canonical global list item library
export const listItems = pgTable(
  ...
  table => [
    unique().on(table.userId, table.normalizedName),
    index("ListItem_userId_idx").on(table.userId),
    index("ListItem_normalizedName_idx").on(table.normalizedName),
    index("ListItem_category_idx").on(table.category),
    index("ListItem_subCategory_idx").on(table.subCategory),
  ]
);

// TaskListItem table - links task items to canonical list item definitions
export const taskListItems = pgTable(
  ...
  table => [
    unique().on(table.taskId, table.listItemId),
    index("TaskListItem_taskId_idx").on(table.taskId),
    index("TaskListItem_listItemId_idx").on(table.listItemId),
  ]
);
...
export const tasksRelations = relations(tasks, ({ one, many }) => ({
  ...
  listTemplate: one(tasks, {
    fields: [tasks.listTemplateId],
    references: [tasks.id],
    relationName: "listTemplate",
  }),
  listInstances: many(tasks, {
    relationName: "listTemplate",
  }),
  taskListItems: many(taskListItems),
}));
...
export const taskListItemsRelations = relations(taskListItems, ({ one }) => ({
  ...
  listItem: one(listItems, {
    fields: [taskListItems.listItemId],
