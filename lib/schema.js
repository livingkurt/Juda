...
export const sections = pgTable(
  "Section",
  {
    ...
    filterTagIds: jsonb("filterTagIds").notNull().default([]),
    filterCompletionTypes: jsonb("filterCompletionTypes").notNull().default([]),
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
    updatedAt: timestamp("updatedAt", { mode: "date" })
      .notNull()
      .defaultNow()
...
export const tasks = pgTable(
  "Task",
  {
    ...
    completionType: text("completionType").notNull().default("checkbox"), // 'checkbox' | 'text' | 'note' | 'workout' | 'goals' | 'reflection'
...
export const taskCompletions = pgTable(
  "TaskCompletion",
  {
    ...
    reflectionAnswers: jsonb("reflectionAnswers"),
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
  },
  table => [
...
// ReflectionQuestion table - versioned reflection prompts per task
export const reflectionQuestions = pgTable(
  "ReflectionQuestion",
  {
    ...
    includeGoalReflection: boolean("includeGoalReflection").notNull().default(false),
    goalReflectionQuestion: text("goalReflectionQuestion"),
  ...
  table => [
    index("ReflectionQuestion_taskId_idx").on(table.taskId),
    index("ReflectionQuestion_dates_idx").on(table.startDate, table.endDate),
  ]
);

// ReflectionGoal table - links reflection tasks to goal tasks
export const reflectionGoals = pgTable(
  "ReflectionGoal",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => generateCuid()),
    reflectionTaskId: text("reflectionTaskId")
      .notNull()
      .references(() => tasks.id, { onDelete: "cascade" }),
    goalTaskId: text("goalTaskId")
      .notNull()
      .references(() => tasks.id, { onDelete: "cascade" }),
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
  },
  table => [
    unique().on(table.reflectionTaskId, table.goalTaskId),
    index("ReflectionGoal_reflectionTaskId_idx").on(table.reflectionTaskId),
    index("ReflectionGoal_goalTaskId_idx").on(table.goalTaskId),
  ]
);
...
export const tasksRelations = relations(tasks, ({ one, many }) => ({
  ...
  reflectionGoalLinks: many(reflectionGoals, {
    relationName: "reflectionLinks",
  }),
  goalReflectionLinks: many(reflectionGoals, {
    relationName: "goalLinks",
  }),
}));
...
export const reflectionQuestionsRelations = relations(reflectionQuestions, ({ one }) => ({
  task: one(tasks, {
    fields: [reflectionQuestions.taskId],
    references: [tasks.id],
  }),
}));

export const reflectionGoalsRelations = relations(reflectionGoals, ({ one }) => ({
  reflectionTask: one(tasks, {
    fields: [reflectionGoals.reflectionTaskId],
    references: [tasks.id],
    relationName: "reflectionLinks",
  }),
  goalTask: one(tasks, {
    fields: [reflectionGoals.goalTaskId],
    references: [tasks.id],
    relationName: "goalLinks",
  }),
}));
