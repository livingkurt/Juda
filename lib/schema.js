...
export const tasks = pgTable(
  "Task",
  {
    ...
    completionType: text("completionType").notNull().default("checkbox"), // 'checkbox' | 'text' | 'selection' | 'note' | 'workout' | 'goal' | 'reflection'
...
// Sleep table - stores daily sleep data from Apple Health/AutoSleep
export const sleepEntries = pgTable(
  "SleepEntry",
  {
    ...
    userId: text("userId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    date: text("date").notNull(), // YYYY-MM-DD (the date this sleep is "for", typically the wake date)
    sleepStart: timestamp("sleepStart", { mode: "date" }), // When sleep began
    sleepEnd: timestamp("sleepEnd", { mode: "date" }), // When sleep ended
    durationMinutes: integer("durationMinutes"), // Total sleep duration in minutes
    source: text("source").default("manual"), // "autosleep", "apple_health", "manual"
    quality: text("quality"), // Optional: "good", "fair", "poor" or numeric score
  ...
  table => [
    index("SleepEntry_userId_idx").on(table.userId),
    index("SleepEntry_date_idx").on(table.date),
    unique("SleepEntry_userId_date_unique").on(table.userId, table.date),
  ]
);
