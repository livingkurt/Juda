...
export const sections = pgTable(
  "Section",
  {
    ...
    startTime: text("startTime"), // Time range start in "HH:MM" format, nullable
    endTime: text("endTime"), // Time range end in "HH:MM" format, nullable
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
    updatedAt: timestamp("updatedAt", { mode: "date" })
      .notNull()
      .defaultNow()
...
export const tasks = pgTable(
  "Task",
  {
    ...
    sectionId: text("sectionId").references(() => sections.id, { onDelete: "cascade" }),
    ...
    priority: text("priority"), // null | 'low' | 'medium' | 'high' | 'urgent'
    startedAt: timestamp("startedAt", { mode: "date" }), // Set when status becomes 'in_progress'
    completionType: text("completionType").notNull().default("checkbox"), // 'checkbox' | 'text' | 'selection' | 'note' | 'workout' | 'goal' | 'reflection' | 'sleep' | 'list'
    ...
    goalData: jsonb("goalData"), // { notes: string, targetDate: string }
    reflectionData: jsonb("reflectionData"), // { questions: ReflectionQuestion[] }
    selectionData: jsonb("selectionData"), // { options: string[] } - Preconfigured dropdown options for selection-type tasks
    sleepData: jsonb("sleepData"), // { targetBedtime: "22:00", targetWakeup: "07:00", targetDurationHours: 8 }
    goalYear: integer("goalYear"), // Year the goal is for (e.g., 2026)
    goalMonths: jsonb("goalMonths"), // Array of months [1,2,3] or null for yearly goals
    folderId: text("folderId").references(() => noteFolders.id, { onDelete: "set null" }),
    sourceTaskId: text("sourceTaskId").references(() => tasks.id, { onDelete: "set null" }),
    rolledFromDate: timestamp("rolledFromDate", { mode: "date" }),
    isRollover: boolean("isRollover").notNull().default(false),
    isOffSchedule: boolean("isOffSchedule").notNull().default(false),
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
    updatedAt: timestamp("updatedAt", { mode: "date" })
      .notNull()
      .defaultNow()
  ...
  table => [index("Task_userId_idx").on(table.userId), index("Task_sourceTaskId_idx").on(table.sourceTaskId)]
);
...
export const taskCompletions = pgTable(
  "TaskCompletion",
  {
    ...
    outcome: text("outcome").notNull().default("completed"), // 'completed' | 'not_completed' | 'rolled_over'
    note: text("note"), // Stores the text input response when completionType is "text"
    selectedOptions: jsonb("selectedOptions").default([]), // Stores array of selected values for selection-type tasks
...
export const tags = pgTable(
  "Tag",
  {
    ...
    scope: text("scope").notNull().default("task"), // "task" | "list" — isolates list tags from task sidebar
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
    updatedAt: timestamp("updatedAt", { mode: "date" })
      .notNull()
      .defaultNow()
...
// WorkoutCycle - Individual training cycle within a program
export const workoutCycles = pgTable(
  "WorkoutCycle",
  {
    id: text("id")
      .primaryKey()
    ...
    name: text("name").notNull().default("Cycle 1"),
  ...
  table => [index("WorkoutCycle_programId_idx").on(table.programId)]
);
...
export const workoutSections = pgTable(
  "WorkoutSection",
  {
    ...
    cycleId: text("cycleId")
      .notNull()
      .references(() => workoutCycles.id, { onDelete: "cascade" }),
    name: text("name").notNull(),
    type: text("type").notNull(), // 'warmup' | 'workout' | 'cooldown' | 'stretches'
    order: integer("order").notNull().default(0),
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
  },
  table => [index("WorkoutSection_cycleId_idx").on(table.cycleId)]
);
...
export const exercises = pgTable(
  "Exercise",
  {
    ...
    goal: text("goal"),
    notes: text("notes"),
    bothSides: boolean("bothSides").notNull().default(false), // For time exercises: run timer twice (left/right)
    order: integer("order").notNull().default(0),
    createdAt: timestamp("createdAt", { mode: "date" }).notNull().defaultNow(),
  },
...
export const tasksRelations = relations(tasks, ({ one, many }) => ({
  ...
  rolloverTasks: many(tasks, {
    relationName: "rollover",
  }),
  listInstance: one(listInstances, {
    fields: [tasks.id],
    references: [listInstances.taskId],
  }),
}));
...
export const tagsRelations = relations(tags, ({ one, many }) => ({
  ...
  taskTags: many(taskTags),
  listItemTags: many(listItemTags),
}));
...
export const workoutSetCompletions = pgTable(
  "WorkoutSetCompletion",
  {
    ...
    outcome: text("outcome"), // null (unchecked) | 'completed' | 'not_completed'
    actualValue: integer("actualValue"), // Actual value achieved (reps, seconds, etc.)
    unit: text("unit"), // Unit for actualValue: 'reps' | 'secs' | 'mins' | 'miles' (preserved from exercise at time of completion)
    completed: boolean("completed").notNull().default(false), // DEPRECATED: kept for backward compatibility, use outcome instead
    value: text("value"), // DEPRECATED: kept for backward compatibility, use actualValue instead
...
export const workoutSectionsRelations = relations(workoutSections, ({ one, many }) => ({
  cycle: one(workoutCycles, {
    fields: [workoutSections.cycleId],
    references: [workoutCycles.id],
  }),
...
// ============================================
// LIST TABLES (Template-based Checklist System)
// ============================================

// ListItem — The Item Library (single source of truth)
export const listItems = pgTable(
  ...
  table => [index("ListItem_userId_idx").on(table.userId)]
);

// ListItemTag — Tags for library items (many-to-many with existing Tag table)
export const listItemTags = pgTable(
  "ListItemTag",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => generateCuid()),
    listItemId: text("listItemId")
      .notNull()
  ...
  table => [
    unique().on(table.listItemId, table.tagId),
    index("ListItemTag_listItemId_idx").on(table.listItemId),
    index("ListItemTag_tagId_idx").on(table.tagId),
  ]
);

// ListTemplate — Template definitions
export const listTemplates = pgTable(
  ...
  table => [index("ListTemplate_userId_idx").on(table.userId)]
);

// ListTemplateItem — Items assigned to a template (junction table)
export const listTemplateItems = pgTable(
  "ListTemplateItem",
  {
    id: text("id")
      .primaryKey()
    ...
    templateId: text("templateId")
      .notNull()
      .references(() => listTemplates.id, { onDelete: "cascade" }),
    listItemId: text("listItemId")
      .notNull()
      .references(() => listItems.id, { onDelete: "cascade" }),
    order: integer("order").notNull().default(0),
  ...
  table => [
    unique().on(table.templateId, table.listItemId),
    index("ListTemplateItem_templateId_idx").on(table.templateId),
    index("ListTemplateItem_listItemId_idx").on(table.listItemId),
  ]
);

// ListInstance — Created from a template, a snapshot for actual use
export const listInstances = pgTable(
  "ListInstance",
  {
    id: text("id")
      .primaryKey()
    ...
    userId: text("userId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    templateId: text("templateId").references(() => listTemplates.id, { onDelete: "set null" }),
  ...
  table => [
    index("ListInstance_userId_idx").on(table.userId),
    index("ListInstance_templateId_idx").on(table.templateId),
    index("ListInstance_taskId_idx").on(table.taskId),
  ]
);

// ListInstanceItem — Snapshot of items in an instance (frozen at creation)
export const listInstanceItems = pgTable(
  "ListInstanceItem",
  {
    id: text("id")
      .primaryKey()
    ...
    instanceId: text("instanceId")
      .notNull()
      .references(() => listInstances.id, { onDelete: "cascade" }),
    listItemId: text("listItemId").references(() => listItems.id, { onDelete: "set null" }),
    name: text("name").notNull(),
    order: integer("order").notNull().default(0),
    quantity: integer("quantity").notNull().default(1),
  ...
  table => [index("ListInstanceItem_instanceId_idx").on(table.instanceId)]
);

// List Relations
export const listItemsRelations = relations(listItems, ({ one, many }) => ({
  user: one(users, { fields: [listItems.userId], references: [users.id] }),
  listItemTags: many(listItemTags),
  templateItems: many(listTemplateItems),
  instanceItems: many(listInstanceItems),
}));

export const listItemTagsRelations = relations(listItemTags, ({ one }) => ({
  listItem: one(listItems, { fields: [listItemTags.listItemId], references: [listItems.id] }),
  tag: one(tags, { fields: [listItemTags.tagId], references: [tags.id] }),
}));

export const listTemplatesRelations = relations(listTemplates, ({ one, many }) => ({
  user: one(users, { fields: [listTemplates.userId], references: [users.id] }),
  templateItems: many(listTemplateItems),
  instances: many(listInstances),
}));

export const listTemplateItemsRelations = relations(listTemplateItems, ({ one }) => ({
  template: one(listTemplates, { fields: [listTemplateItems.templateId], references: [listTemplates.id] }),
  listItem: one(listItems, { fields: [listTemplateItems.listItemId], references: [listItems.id] }),
}));

export const listInstancesRelations = relations(listInstances, ({ one, many }) => ({
  user: one(users, { fields: [listInstances.userId], references: [users.id] }),
  template: one(listTemplates, { fields: [listInstances.templateId], references: [listTemplates.id] }),
  task: one(tasks, { fields: [listInstances.taskId], references: [tasks.id] }),
  instanceItems: many(listInstanceItems),
}));

export const listInstanceItemsRelations = relations(listInstanceItems, ({ one }) => ({
  instance: one(listInstances, { fields: [listInstanceItems.instanceId], references: [listInstances.id] }),
  listItem: one(listItems, { fields: [listInstanceItems.listItemId], references: [listItems.id] }),
}));

// Sleep table has been REMOVED - sleep data now stored in TaskCompletions with selectedOptions field
