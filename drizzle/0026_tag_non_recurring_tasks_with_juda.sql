...
BEGIN
  ...
  FOR v_user_record IN SELECT id FROM "User" LOOP
    ...
    RAISE NOTICE 'Processing user: %', v_user_record.id;
    ...
    -- Count matching tasks for debugging
    ...
    RAISE NOTICE 'Found % non-recurring tasks with completions for user %', v_task_count, v_user_record.id;
    ...
    LOOP
      ...
      IF NOT EXISTS (
        ...
        RAISE NOTICE 'Tagged task % with "Juda" tag', v_task_record.id;
      ELSE
        RAISE NOTICE 'Task % already has "Juda" tag, skipping', v_task_record.id;
      END IF;
    END LOOP;
  END LOOP;
