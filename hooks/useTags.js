...
export function useTags() {
  ...
  // Create a new tag
  const createTag = useCallback(async (name, color = "#6366f1") => {
    try {
      const response = await fetch("/api/tags", {
        method: "POST",
  ...
  const removeTagFromTask = useCallback(async (taskId, tagId) => {
    try {
      const response = await fetch(`/api/task-tags?taskId=${taskId}&tagId=${tagId}`, {
        method: "DELETE",
      });
